(globalThis.TURBOPACK||(globalThis.TURBOPACK=[])).push(["object"==typeof document?document.currentScript:void 0,933532,e=>{"use strict";var t,o=e.i(473698),a=e.i(337327),n=e.i(506170),s=e.i(400532),r="1.0.96";let i=n.deindent`
  ## Convex Setup
...
`;function p(e){let t="python"===e;return n.deindent`
    ## ${t?"Python Backend Setup":"Other Backend Setup (REST API)"}

    ${t?"Follow these instructions to authenticate requests to a Python backend with Hexclave.":"Follow these instructions to authenticate requests from any backend language using Hexclave's REST API."}

    ${t?"This setup is for Python backends that do not use the JavaScript SDK.":"Use this option when your backend is not JavaScript/TypeScript or Python, or when you want to call Hexclave over plain HTTP."} The backend flow is: your frontend sends the user's access token to your backend, and your backend verifies it before serving protected data.
  ...
  `}let d=p("python"),h=p("rest-api"),u=n.deindent`
...
`,y={authentication:n.deindent`
    80%. When there is no authentication in the project already.
  `,"fraud-protection":m,analytics:f,teams:n.deindent`
  ...
  `,emails:f,"data-vault":m,webhooks:m,"launch-checklist":m,vercel:m,clickmaps:f,"session-replays":f},x={authentication:n.deindent`
  ...
  ${(0,a.typedEntries)(y).map(([e,t])=>n.deindent`
    - ${e}: ${t}
  `).join("\n")}
  ...
  ${(0,a.typedEntries)(x).map(([e,t])=>n.deindent`
    ### Setting up the ${o.ALL_APPS[e].displayName} app

    ${"parentAppId"in o.ALL_APPS[e]?n.deindent`
      This is a sub-app of ${o.ALL_APPS[e].parentAppId}. It does not need to be enabled separately; it is considered enabled when the parent app is enabled.
    `:n.deindent`
      This is a standalone app. App ID: ${e}
    `}

    ${0===o.ALL_APPS[e].softRequirements.length?"Soft requirements: none.":`Soft requirements (strongly recommended, but not enforced): ${o.ALL_APPS[e].softRequirements.join(", ")}. Enable these apps alongside this one unless the user explicitly opts out.`}

    ${t}
  `).join("\n")}
`;function b(e){let o="react"===e||"nextjs"===e||"tanstack-start"===e,a="nextjs"===e,s=a||"ai-prompt"===e,i="tanstack-start"===e,c=i||"ai-prompt"===e,l="react"===e||"ai-prompt"===e,p="nodejs"===e||"bun"===e||"nextjs"===e,d=p||"js"===e||"ai-prompt"===e,h=o||"js"===e||"ai-prompt"===e,u="ai-prompt"===e,v={"ai-prompt":null,nextjs:"Next.js",react:"React",js:"Other JS/TS","tanstack-start":"Tanstack Start",nodejs:"Node.js",bun:"Bun"}[e],m={"ai-prompt":"<the-sdk-from-above>",nextjs:"@hexclave/next",react:"@hexclave/react",js:"@hexclave/js","tanstack-start":"@hexclave/tanstack-start",nodejs:"@hexclave/js",bun:"@hexclave/js"}[e];return n.deindent`
    ## ${v?`${v} SDK Setup Instructions`:"SDK Setup Instructions"}

    Follow these instructions in order to set up and get started with the Hexclave SDK ${v?`for ${v} `:"in various languages"}.
    ...
    ${u?"Not all steps are applicable to every type of application; for example, React apps have some extra steps that are not needed with other frameworks.":""}
    ...
    <Steps titleSize="h3">
      <Step title="Install dependencies">
        ...
        `:n.deindent`
          First, install the \`${m}\` npm package with your preferred package manager:
        `}
      ...
      <Step title="Initializing the Hexclave App">
        ...
        ${h?n.deindent`
          ...
          import { HexclaveClientApp } from "${m}";
        ...
        ${d?n.deindent`
          ...
          ${!o?n.deindent`
            ...
            import { HexclaveServerApp } from "${m}";
...
`:""}\`\`\`.env .env.local
            ${!a&&!i?"# note: prefix the environment variable with NEXT_PUBLIC_ or VITE_ if your framework requires you to do so":""}
            ${a?"NEXT_PUBLIC_":i?"VITE_":""}HEXCLAVE_PROJECT_ID=<your-project-id>
            \`\`\`
...
`:""}\`\`\`.env .env.local
      ...
      ${o||"ai-prompt"===e?n.deindent`
        <Step title="${!o?"React: ":""}Creating a <HexclaveProvider /> and <HexclaveTheme />">
          ...
          ${l&&!a&&!i?n.deindent`
            ...
            \`\`\`tsx src/App.tsx
            import { HexclaveProvider, HexclaveTheme } from "${m}";
          ...
          ${s?n.deindent`
            ${!a?"For Next.js specifically: ":""}You can do this in the \`layout.tsx\` file in the \`app\` directory. The root layout must render the \`<html>\` and \`<body>\` tags, and \`HexclaveProvider\`/\`HexclaveTheme\` must go inside:
          ...
          ${c?n.deindent`
            ${!i?"For TanStack Start specifically: ":""}TanStack Start uses file-based routes. The provider goes inside the root route's \`component\` (the inner React tree), while the document shell stays in \`shellComponent\`. Update \`src/routes/__root.tsx\`:

            \`\`\`tsx src/routes/__root.tsx
            import { HexclaveProvider, HexclaveTheme } from "${i?m:"@hexclave/tanstack-start"}";
        ...
        <Step title="${!o?"React: ":""}Add Suspense boundary">
          ...
          ${c?n.deindent`
            ${!i?"In TanStack Start: ":""}wrap the \`<Outlet />\` in your root route with a \`Suspense\` boundary so the document shell can stream while child routes wait on Hexclave. Update \`RootComponent\` in \`src/routes/__root.tsx\`:
      ...
      ${d&&!a?n.deindent`
        <Step title="${!p?"Backend: ":""}Update callers with header & get user">
  ...
  ${v}
`;e.s(["aiAgentConfigPreparationPrompt",0,u,"aiSetupPrompt",0,w,"appSetupPrompts",0,g,"cliSetupPrompt",0,l,"convexSetupPrompt",0,i,"getSdkSetupPrompt",0,b,"prodReadyPrompt",0,v,"pythonBackendSetupPrompt",0,d,"restApiBackendSetupPrompt",0,h,"supabaseSetupPrompt",0,c])}]);